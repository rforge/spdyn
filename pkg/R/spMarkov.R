    ...
    function(data,listw,stateVars,n.states,stateNames=NULL,style='quantile',
        ...
        if(pool==FALSE){
            ...
            for(i in 1:(t-1)){
                ...
                brks[1,1]<-brks[1,1]-brks[1,1]/1e+10
                brks[1,n.states+1]<-brks[1,n.states+1]+brks[1,n.states+1]/1e+10
                
                brks[2,]<-quantile(x[,i],seq(from=0,to=1,length.out=n.states+1))
                
                brks[2,1]<-brks[2,1]-brks[2,1]/1e+10
                brks[2,n.states+1]<-brks[2,n.states+1]+brks[2,n.states+1]/1e+10
                
                brks[3,]<-quantile(x[,i+1],seq(from=0,to=1,length.out=n.states+1))
                brks[3,1]<-brks[3,1]-brks[3,1]/1e+10
                brks[3,n.states+1]<-brks[3,n.states+1]+brks[3,n.states+1]/1e+10
